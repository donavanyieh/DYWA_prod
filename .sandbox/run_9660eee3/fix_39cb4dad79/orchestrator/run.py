...
from orchestrator.dashboard.build_bundle import write_dashboard_bundle, write_dashboard_index
from orchestrator.verifier.run import verify_batch_to_files
...
def run_live(config: RunConfigV1) -> Path:
    ...
    try:
        ...
        verifier_results = verify_batch_to_files(
            bug_report_paths=bug_report_paths,
            output_root=run_dir / "verifier",
        ...
        for verifier_result in verifier_results:
            ...
            log(
                ...
                f"{decision.report_id}: {decision.classification.value} "
