...
def classify_report(
    *,
    ...
    classification = VerifierClassification(str(decision["classification"]))
    canonical_bug_id = decision.get("canonical_bug_id")
    if classification == VerifierClassification.CONFIRMED and not canonical_bug_id:
        canonical_bug_id = slugify(report.title)

    required_next_action = VerifierNextAction(str(decision["required_next_action"]))
    return VerifierDecisionV1(
        ...
        reasoning_summary=str(decision["reasoning_summary"]),
